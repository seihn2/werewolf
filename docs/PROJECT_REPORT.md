...
> **项目状态**：完整的可运行工程产品；对战、自博弈、潜在策略聚类、抽象博弈、Deep CFR、CFR 偏好数据、DPO/LoRA 训练、多轮编排、双阵营对照评测与 WolfPlay Studio 产品界面均已具备。
...
| DPO/LoRA 训练入口 | 已实现 | 支持 TRL `DPOTrainer`、LoRA 与全参数训练配置 |
...
全参数训练增加 `--no-lora`。训练代码会生成 `training_manifest.json`，记录数据、模型、超参数、随机种子和依赖版本。
...
> 快照日期：2026-08-06。最终测试数量与静态检查结果以本节记录的命令输出为准。
...
| Deep CFR | 真实 Torch 冒烟通过 | 一轮 External-Sampling、网络更新、checkpoint 加载和平均策略采样 |
...
| `head-to-head` | 已完成启发式烟测 | 验证双方阵营互换和统计结构 |
...
启发式 Planner 定义了 `fake_seer_claim`、伪造验人和组织票型等候选，并由 Evaluator 结合身份、公开证据和局势进行评分。自博弈轨迹会保存这些候选及其结果，随后可通过 CFR-DPO 数据和 DPO 训练进入模型策略。
...
狼人和村民阵营的任务、信息和天然胜率不同。双阵营互换可以分别报告 challenger 的狼人侧和村民侧表现，并减少固定阵营带来的评测偏差。

### Q10：固定 seed 是否能保证模型评测完全重复？

固定 seed 可以稳定角色分配、平票处理和启发式决策等本地随机过程。外部模型还可以通过固定服务版本、解码参数，并记录完整请求和响应元数据来增强可重复性。
...
可以固定模型和数据版本，运行多轮自博弈、聚类、Deep CFR 与 DPO；将训练 checkpoint 接入下一轮模型后端；记录 regret、策略熵、候选覆盖率和模型错误；最后使用同 seed 镜像对战和置信区间分析策略变化。

### Q14：如何进行稳定的对照评测？

使用同一基础模型和同一批环境 seed 做阵营镜像配对，每个阵营运行足够局数；报告平局处理、95% 置信区间、多个重复 seed、超时和回退率；预先固定统计方案，并保留原始对局和训练 manifest。
...
项目最有价值的部分是**代码链路完整且可解释**：即使没有外部模型，也能离线运行、聚类、遍历抽象状态并生成数据；安装训练依赖后，可以执行网络更新、保存 checkpoint、构造 CFR 偏好并接入下一轮策略迭代。
